...
def gradf(x,delta,wl):
    return 2*x - 2*math.pi * delta * math.sin(2*math.pi*x / wl) / wl

@cuda.jit(device=True)
def lossf(x,delta,wl):
    return x**2 + delta * math.cos(2*math.pi*x / wl)

# Define a kernel that uses the device function
@cuda.jit
def GD(tensor, delta_span, wl_span, x, steps):
...
def onegpu(delta_span, wl_span, x,steps, nn, bound = 1e+16):
    ...
    GD[(numblocks,num_grid,num_grid), 2**10](tensord, delta_spand, wl_spand, x, steps)
...
if __name__ == '__main__':
    ...
    x = float64(5.0) 
    num_grid = 10
    delta_span = np.linspace(0.01,0.2,num_grid,dtype=np.float64)
    ...
    dictosave = {"numB": numB}
    savemat("gpuexp-4-2.mat", dictosave)